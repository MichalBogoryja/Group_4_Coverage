...
public:
    ...
    PointCloudRGBCombiner(ros::NodeHandle nh): nh_(nh), param_nh("~") // ,colored_pcl_in(0L)
    {
        // std::string image_topic = nh_.resolveName(“img_in”);
        // std::string image_topic = "/rgb/image_rect_color";
        image_topic = getParam<std::string>(param_nh, "image_topic", "/rgb/image_rect_color"); 
        pcl_topic = getParam<std::string>(param_nh, "pcl_topic", "/depth/pcd");
        pcd_color_topic = getParam<std::string>(param_nh, "pcd_color_topic", "/depth/pcd_colored");
        pcd_rect_topic = getParam<std::string>(param_nh, "pcd_rect_topic", "/depth/pcd_rect");
        far_clip = getParam<double>(param_nh, "far_clip", 3.5);
